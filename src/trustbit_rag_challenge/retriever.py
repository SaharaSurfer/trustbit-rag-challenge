...
class ChromaRetriever:
    ...
    This class handles connecting to the vector store, loading the company-to-SHA1 
    mapping, and performing similarity searches with metadata filtering.
    ...
    def __init__(self) -> None:
        ...
        Sets up the embedding model, connects to the existing ChromaDB collection,
        and loads the company mapping into memory.
        """
    ...
    def _load_company_map(self) -> Dict[str, str]:
        ...
        -------
        Dict[str, str]
            A dictionary where keys are company names and values are SHA1 strings.
        ...
        if not self.mapping_csv.exists():
            ...
            raise FileNotFoundError(f"Mapping CSV not found at {self.mapping_csv}")

        try:
            df = pd.read_csv(self.mapping_csv)
            if "company_name" not in df.columns or "sha1" not in df.columns:
                raise ValueError("CSV must contain 'company_name' and 'sha1' columns")
    ...
    def get_sha1_by_name(self, company_name: str) -> Optional[str]:
        """
    ...
    def retrieve(
        ...
        """
        Perform a semantic search for the query, filtered by the company's document.
        ...
        if target_sha1:
            ...
            logger.debug(f"Filtering by company: {company_name} ({target_sha1})")
        else:
            logger.warning(f"Company '{company_name}' not found in mapping. Searching globally.")
